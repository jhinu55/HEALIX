...
CREATE OR REPLACE FUNCTION check_doctor_exists(doctor_id uuid) 
RETURNS boolean AS $$
BEGIN
    RETURN EXISTS (SELECT 1 FROM doctors WHERE id = doctor_id);
END;
$$ LANGUAGE plpgsql;

-- Create function to check if an assistant exists
CREATE OR REPLACE FUNCTION check_assistant_exists(assistant_id uuid) 
RETURNS boolean AS $$
BEGIN
